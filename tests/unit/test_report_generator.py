...
def mock_structured(tmp_path):
    ...
    ms = MagicMock()
    ms.list_recent.return_value = [DummyRec("r1", "foo").to_dict(), DummyRec("r2", "bar").to_dict()]
...
def mock_vector(tmp_path):
    ...
    mv.query_similar.return_value = [
        {"case_id": "r1", "text": "foo", "entities": {"people": ["Alice"], "wallet_addresses": ["0x1"]}},
        {"case_id": "r2", "text": "bar", "entities": {"people": ["Bob"], "wallet_addresses": ["0x2"]}},
    ]
    return mv
