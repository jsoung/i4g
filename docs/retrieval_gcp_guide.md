...
This guide documents the Google Cloud workflow for the Retrieval proof of concept. It covers the Terraform-provisioned Discovery data store, the Python ingestion and query helpers, and day-to-day troubleshooting. Keep this separate from the local sandbox guide to avoid mixing laptop-only steps with hosted services.

---

## Prerequisites

- `infra/environments/dev` Terraform stack applied (creates the Discovery data store `retrieval-poc`).
...
- Each document now sets `Document.content` with UTF-8 raw bytes and `mime_type="text/plain"`, satisfying Discovery’s unstructured content requirement.
